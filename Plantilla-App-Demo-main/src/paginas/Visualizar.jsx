...
const Visualizar = () => {
    const {model, handleModel} = useContext(tratamientosContext)
    const { id } = useParams()
            ...
            <div>
                {
                    Object.keys(paciente).length != 0 ?
                        (
                            ...
                            <button className="px-5 py-2 bg-green-800 text-white rounded-lg hover:bg-green-700" onClick={handleModel}>Registrar</button>
                            {
                                model && <ModalTratamiento idPaciente={paciente._id} />
                            }
                            </>
